...
name: tamagui-v2
description: |
  Universal React UI framework for web and native (v2.0.0-rc+). Use when building cross-platform apps with React Native + Web,
  creating styled components with `styled()`, configuring design tokens/themes, using Tamagui UI components
  (Button, Dialog, Sheet, Input, Select, Tabs, Progress, Card, Accordion, Avatar, etc.), working with animations, or
  needing Bento/Takeout premium components.
  Triggers: "tamagui", "universal UI", "react native web", "styled()", "design tokens", "$token",
  "XStack/YStack/ZStack", "useTheme", "useMedia", "@tamagui/*" imports, "Progress", "Separator", "SizableText",
  "Heading", "Spinner", "Card", "Avatar", "ListItem", "ToggleGroup", "RadioGroup", "Slider", "Accordion",
  "AlertDialog", "Toast", "Tooltip", "bento", "takeout", "one.js router", "lucide-icons",
  "createStyledContext", "withStaticProperties", "@tanstack/react-table", "zero sync", "better auth".
---

# Tamagui v2 Skill

> Universal React UI framework for web and native (v2.0.0-rc+)
...
- **50+ cross-platform components** - Button, Dialog, Sheet, Input, Select, Tabs, Progress, Card, Accordion, Avatar, Toast, Tooltip, and more
- **`tamagui` umbrella package** - Single import for ALL UI components (preferred over individual `@tamagui/*` packages)
...
**Always prefer importing from `'tamagui'`** (the umbrella package) instead of individual packages:

```tsx
// ✅ Preferred: umbrella import
import { Button, Input, Progress, YStack, SizableText, Spinner, Theme } from 'tamagui'

// ❌ Avoid: individual package imports (unless tree-shaking is critical)
import { Button } from '@tamagui/button'
import { Input } from '@tamagui/input'
```

Exception: `LinearGradient` is imported from `'tamagui/linear-gradient'`.
