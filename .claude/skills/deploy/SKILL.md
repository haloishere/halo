...
```bash
# List recent builds
gcloud builds list --region=europe-west1 --project=halo-493622 --limit=3 --format="table(id,status,createTime,substitutions._ENVIRONMENT)"

# Stream logs for the latest build
gcloud builds log $(gcloud builds list --region=europe-west1 --project=halo-493622 --limit=1 --format="value(id)") --region=europe-west1 --project=halo-493622 --stream
```
...
| Watch Cloud Build (API/CMS) | `gcloud builds list --region=europe-west1 --project=halo-493622 --limit=3` |
