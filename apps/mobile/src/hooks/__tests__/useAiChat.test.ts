...
describe('useAiChat — lazy conversation creation (`/chat/new` sentinel)', () => {
  it('creates a conversation before sending when conversationId is "new"', async () => {
    ...
    // The create mutation fires with the topic-defaulted body (title is
    // auto-generated by the backend later via autoTitleConversation). The
    // hardcoded `food_and_restaurants` topic is a Phase-1 placeholder; it
    // becomes the user-picked value once the Phase-4 scenario picker lands.
    expect(mockCreateMutateAsync).toHaveBeenCalledTimes(1)
    expect(mockCreateMutateAsync).toHaveBeenCalledWith({ topic: 'food_and_restaurants' })

    // Guard against Phase 4 slipping silently past release: the hardcoded
    // fallback fires a console.warn so the tag is greppable in prod logs
    // and hitting the code path is observable.
    const warnSpy = console.warn as unknown as ReturnType<typeof vi.fn>
    expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('phase-4-scenario-picker'))
